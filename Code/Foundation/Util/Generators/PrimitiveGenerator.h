...
namespace Util
{
	...
	protected:
		template <typename T>
		std::shared_ptr<Graphics::Mesh> Store(const std::vector<T>& vertexData, const std::vector<uint32_t>& elements,
	...
	template <typename T>
	std::shared_ptr<Graphics::Mesh> PrimitiveGenerator::Store(const std::vector<T>& vertexData,
