...
const float outlineEdge = 0.05;

void main()
{
    float distance = 1.0f - texture(fontMapSampler, textureCoords).r;
    float fontAlpha = 1.0f - smoothstep(fontWidth, fontWidth + fontEdge, distance);
    float edgeAlpha = 1.0f - smoothstep(outlineWidth, outlineWidth + outlineEdge, distance);
